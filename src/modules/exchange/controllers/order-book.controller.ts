...
export class OrderBookController {
  ...
  async getMarketIds(): Promise<string[]> {
    return await this.orderBookService.getMarketIds();
  }
  ...
  async getBestBid(
    ...
    const bestBid = await this.orderBookService.getBestBid(marketId);
    if (!bestBid) {
      throw new HttpException(
  ...
  async getBestAsk(
    ...
    const bestAsk = await this.orderBookService.getBestAsk(marketId);
    if (!bestAsk) {
      throw new HttpException(
  ...
  async getSpread(
    ...
    const spread = await this.orderBookService.getSpread(marketId);
    if (spread === null) {
      throw new HttpException(
